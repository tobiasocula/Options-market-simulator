...
def cross_excitation(params: CrossExcitation, save=False, savedir=None):
    ...
    def implied_vol_call(C_mkt, S, K, T, r, q, max_tries=3):
        if C_mkt < 1e-4:  # Too cheap
            return None
        ...
        def bs_diff(sigma):
            try:
                ...
                raise AssertionError(f"failed to call black scholes")
    ...
    def compute_intensities(trades, assetprice, time, M, N, T_current):

        intensities = np.zeros((M, N, 2))
        for this_exp, this_strike, this_type in itertools.product(range(M), range(N), range(2)):

            exc = params.mu_intensity

            for exp_i, strike_i, type_i in itertools.product(range(M), range(N), range(2)):
                cur_trades = trades[exp_i, strike_i, type_i, T_current] if trades[exp_i, strike_i, type_i, T_current] is not None else None
                dT = abs(params.expiry_dts[exp_i] - params.expiry_dts[this_exp])
                dm = abs(np.log(params.strike_prices[strike_i] / assetprice) - np.log(params.strike_prices[this_strike] / assetprice))
                if cur_trades is None:
                    # no trades means no excitation
                    continue
                else:
                    for entry in cur_trades:
                        elapsed = time - entry["time"]
                        print('elapsed time:', elapsed, "with entry[time] being", entry['time'])
                        kernel = np.exp(params.w * np.log(entry["volume"] + 1e-8) - params.gamma_t * dT - params.gamma_m * dm - params.beta * elapsed)
                        # print('kernel part 1:', params.w * np.log(entry["volume"] + 1e-8))
                        # print('kernel part 2:', params.gamma_t * dT)
                        # print('kernel part 3:', params.gamma_m * dm)
                        # print('kernel part 4:', params.beta * elapsed)
                        # print('kernel:', kernel)
                        exc += kernel * params.rho_self * params.tau[type_i][this_type]

            intensities[this_exp, this_strike, this_type] = exc
    ...
    for T_current in range(1, T):
        ...
        #intensities = compute_intensities(trades, assetdata[0, T_current], params.dt * T_current, M, N, T_current - 1) # (M, N, 2)

        excitations *= np.exp(-params.beta * params.dt)
        ...
        for this_exp, this_strike, this_type in itertools.product(range(M), range(N), range(2)):
            ...
            for entry in this_trades:
                for target_exp, target_strike, target_type in itertools.product(range(M), range(N), range(2)):
                    dT = abs(params.expiry_dts[source_exp] - params.expiry_dts[target_exp])
                    dm = abs(np.log(params.strike_prices[source_strike] / assetdata[0, T_current]) -
                            np.log(params.strike_prices[target_strike] / assetdata[0, T_current]))

                    kernel = (
                        np.exp(params.w * np.log(volume + 1e-8)
                            - params.gamma_t * dT
                            - params.gamma_m * dm)
                    )

                    excitations[target_exp, target_strike, target_type] += (
                        params.rho_self
                        * params.tau[source_type, target_type]
                        * kernel
                    )
        ...
        #intensities_keep[:, :, :, T_current] = intensities
        Lambda = 0
        lambda_keep[T_current] = Lambda

         # 3. Discrete Poisson: num_events ~ Poisson(Lambda * dt)
        ...
        for _ in range(num_events):
            ...
            probs_per_contract = intensities / Lambda # same shape as intensities_prime
            ...
            buy = np.random.uniform() <= 0.5 # temporarily
    ...
    if save:
        ...
        return orderbooks, assetdata, overviews, trades, all_trades, intensities_keep, lambda_keep, num_events_keep
