...
num_samples = 50
#PARAM_RANGE = np.linspace(1e-6, 1e-4, 50) # mu
#PARAM_RANGE = np.linspace(0.2, 0.9, 50) # beta
PARAM_RANGE = np.linspace(0.005, 0.025, 50) # alpha m

dmode = 1
debugger = Debugger(mode=dmode)

json_path = Path.cwd() / "nn_learning_intensity" / "trainset_alpha_m" / "param_info.json"
...
for idx, val in enumerate(PARAM_RANGE):
    ...
    save = Path.cwd() / "nn_learning_intensity" / "trainset_alpha_m" / f"set_{idx}"
